...
func TestPrivateKeySerialize(t *testing.T) {
	for i, test := range privateKeyTests {
		...
		if err != nil {
			t.Errorf("#%d: serialize of private key failed: %s", i, err)
			continue
		}
		serializedHex := hex.EncodeToString(buf.Bytes())

		// First 3 bytes differ due too old format vs. new format
		if serializedHex[6:] != test.privateKeyHex[6:] {
			t.Errorf("#%d: serialized private key differs from original data: %s != %s", i, serializedHex, test.privateKeyHex)
			continue
		}

		rereadPacket, err := Read(readerFromHex(serializedHex))
		if err != nil {
			t.Errorf("#%d: failed to parse: %s", i, err)
			continue
		}

		rereadPrivKey := rereadPacket.(*PrivateKey)
		if !privKey.Encrypted {
			t.Errorf("#%d: reread private key isn't encrypted", i)
			continue
		}

		err = rereadPrivKey.Decrypt([]byte("testing"))
		if err != nil {
			t.Errorf("#%d: failed to decrypt reread key: %s", i, err)
			continue
		}
	}
}
